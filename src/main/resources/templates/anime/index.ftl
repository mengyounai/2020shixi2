...
<script>
    $(function () {
        var initialPreview =[];
        if ('${(animeInfo.animeIcon)!""}'!=''){
            initialPreview="<img class='kv-preview-data file-preview-image'src='${(animeInfo.animeIcon)!""}'>"
        }
        ...
        $('#input-id').on('fileuploaded', function(event,data,previewId,index){
            ...
            $('#animeIcon').val(data.response.data.fileUrl)
        });
    });
    ...
    <div class="container-fluid">
        <div class="row clearfix">
            <div class="col-md-12 column">
                <form role="form" method="post" action="/bangumi/admin/anime/save">
                    <div class="form-group">
                        <label for="exampleInputEmail1">名称</label><input name="animeName" type="text" class="form-control" value="${(animeInfo.animeName)!''}"/>
                    </div>
                    <div class="form-group">
                        <label for="exampleInputPassword1">上映时间</label><input name="animeTime" type="text" class="form-control" value="${(animeInfo.animeTime)!''}"/>
                    </div>
                    <div class="form-group">
                        <label for="exampleInputPassword1">作者</label><input name="animeAuthor" type="text" class="form-control" value="${(animeInfo.animeAuthor)!''}"/>
                    </div>
                    <div class="form-group">
                        <label for="exampleInputPassword1">描述</label><input name="animeDescription" type="text" class="form-control" value="${(animeInfo.animeDescription)!''}"/>
                    </div>
                    <div class="form-group">
                        <label for="exampleInputPassword1">主角</label><input name="animeCharacter" type="text" class="form-control" value="${(animeInfo.animeCharacter)!''}"/>
                    </div>
                    <div class="form-group" style="width: 400px;height: 300px;">
                        <label>图片</label><input id="animeIcon" name="animeIcon" type="text" hidden="hidden" value="${(animeInfo.animeIcon)!''}">
                        ...
                        <select name="labelType" class="form-control">
                                <#list labelList as labelList>
                                    <option value="${labelList.labelType}"
                                            <#if (animeInfo.labelType)?? && animeInfo.labelType==labelList.labelType>
                                                selected
                                            </#if>
                                    >${labelList.labelName}
                                    </option>
                    ...
                    <input hidden type="text" name="animeId" value="${(animeInfo.animeId)!''}">
