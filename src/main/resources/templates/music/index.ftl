...
<script>
    $(function () {
        var initialPreview =[];
        if ('${(musicInfo.musicIcon)!""}'!=''){
            initialPreview="<img class='kv-preview-data file-preview-image'src='${(musicInfo.musicIcon)!""}'>"
        }
        ...
        $('#input-id').on('fileuploaded', function(event,data,previewId,index){
            ...
            $('#musicIcon').val(data.response.data.fileUrl)
        });
    });
    ...
    <div class="container-fluid">
        <div class="row clearfix">
            <div class="col-md-12 column">
                <form role="form" method="post" action="/bangumi/admin/music/save">
                    <div class="form-group">
                        <label for="exampleInputEmail1">名称</label><input name="musicName" type="text" class="form-control" value="${(musicInfo.musicName)!''}"/>
                    </div>
                    <div class="form-group">
                        <label for="exampleInputPassword1">上映时间</label><input name="musicTime" type="text" class="form-control" value="${(musicInfo.musicTime)!''}"/>
                    </div>
                    <div class="form-group">
                        <label for="exampleInputPassword1">作者</label><input name="musicAuthor" type="text" class="form-control" value="${(musicInfo.musicAuthor)!''}"/>
                    </div>
                    <div class="form-group">
                        <label for="exampleInputPassword1">描述</label><input name="musicDescription" type="text" class="form-control" value="${(musicInfo.musicDescription)!''}"/>
                    </div>
                    <div class="form-group" style="width: 400px;height: 300px;">
                        <label>图片</label><input id="musicIcon" name="musicIcon" type="text" hidden="hidden" value="${(musicInfo.musicIcon)!''}">
                        ...
                        <select name="labelType" class="form-control">
                                <#list labelList as labelList>
                                    <option value="${labelList.labelType}"
                                            <#if (musicInfo.labelType)?? && musicInfo.labelType==labelList.labelType>
                                                selected
                                            </#if>
                                    >${labelList.labelName}
                                    </option>
                    ...
                    <input hidden type="text" name="musicId" value="${(musicInfo.musicId)!''}">
