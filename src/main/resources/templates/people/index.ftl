...
<script>
    $(function () {
        var initialPreview =[];
        if ('${(peopleInfo.peopleIcon)!""}'!=''){
            initialPreview="<img class='kv-preview-data file-preview-image'src='${(peopleInfo.peopleIcon)!""}'>"
        }
        ...
        $('#input-id').on('fileuploaded', function(event,data,previewId,index){
            ...
            $('#peopleIcon').val(data.response.data.fileUrl)
        });
    });
    ...
    <div class="container-fluid">
        <div class="row clearfix">
            <div class="col-md-12 column">
                <form role="form" method="post" action="/bangumi/admin/people/save">
                    <div class="form-group">
                        <label for="exampleInputEmail1">名称</label><input name="peopleName" type="text" class="form-control" value="${(peopleInfo.peopleName)!''}"/>
                    </div>
                    <div class="form-group">
                        <label for="exampleInputPassword1">生日</label><input name="peopleBirthday" type="text" class="form-control" value="${(peopleInfo.peopleBirthday)!''}"/>
                    </div>
                    <div class="form-group">
                        <label for="exampleInputPassword1">性别</label><input name="peopleSex" type="text" class="form-control" value="${(peopleInfo.peopleSex)!''}"/>
                    </div>
                    <div class="form-group">
                        <label for="exampleInputPassword1">描述</label><input name="peopleDescription" type="text" class="form-control" value="${(peopleInfo.peopleDescription)!''}"/>
                    </div>
                    <div class="form-group">
                        <label for="exampleInputPassword1">出演动画</label><input name="animeId" type="text" class="form-control" value="${(peopleInfo.animeId)!''}"/>
                    </div>
                    <div class="form-group" style="width: 400px;height: 300px;">
                        <label>图片</label><input id="peopleIcon" name="peopleIcon" type="text" hidden="hidden" value="${(peopleInfo.peopleIcon)!''}">
                        <div class="file-loading" >
                            <input id="input-id" type="file" > <p>支持jpg、jpeg、png、gif格式，大小1M</p>
                        </div>
                    </div>

                    <input hidden type="text" name="peopleId" value="${(peopleInfo.peopleId)!''}">
                    <button type="submit" class="btn btn-default" style="margin-top: 120px;">提交</button>
                </form>
            </div>
        </div>
    </div>
